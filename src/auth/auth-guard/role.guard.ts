...
// Enhanced Role-based Guard with better error handling
...
export class EnhancedRolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!hasRole) {
      throw new ForbiddenException(`Access denied. Required roles: [${requiredRoles.join(', ')}]. User role: ${user.role.name}`);
    }
