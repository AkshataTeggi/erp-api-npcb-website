...
async createContact(createContactDto: CreateContactDto) {
  try {
    ...
    const createdAtPST = moment().tz('America/Los_Angeles').format('YYYY-MM-DD hh:mm A z');
  ...
  try {
    await this.emailService.sendEmail(
      ...
      'contact_customer',
      { name, mobile, message, email, company,  createdAtPST  }
    );
    ...
    if (emailRecipients.length > 0) {
      try {
        await this.emailService.sendEmail(
          ...
          'New Message Arrived',
          'contact_admin',
          { name, email, mobile, message, company,  createdAtPST  }
        );
